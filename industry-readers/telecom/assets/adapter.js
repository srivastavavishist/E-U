(function() {
    ...
    const companies = (d.companies || []).map(c => {
        const rep = c.latest_brief ? [{
            date: c.latest_brief.brief_date || d.meta.publication_date || "2026-07-01",
            type: 'Brief',
    ...
    if (d.pilot_industries) {
        Object.keys(d.pilot_industries).forEach(sectorId => {
            ...
            if (pi.trends) {
                pi.trends.forEach((t, i) => {
                    intel.push({
                        id: t.trend_id || ('t' + Math.random()),
                        type: 'Macro Trend',
                        date: d.meta.publication_date || "2026-07-01",
                        sectors: [sectorId],
    ...
    if (intel.length === 0) {
        (d.industry_cards || []).forEach((c, i) => {
            intel.push({
                id: 'intel-' + c.id + '-' + i,
                type: 'Sector Readout',
                date: c.readout_as_of || d.meta.publication_date || "2026-07-01",
                sectors: [c.id],
